...
```
There are `r length(get_significant_results(Indep.model.10))` discoveries, which is **more** than the `mashcommonbaseline` model. The covariance structure found here is:
```{r}
...
```{r, echo=FALSE}
delta = data$C %*% t(L.1)
barplot(c(sqrt(mean((delta - mashcontrast.model.1$result$PosteriorMean)^2)/mean((delta - data$Chat%*%t(L.1))^2)), sqrt(mean((delta - Indep.model.1$result$PosteriorMean)^2)/mean((delta - data$Chat%*%t(L.1))^2))), ylim=c(0,0.2), names.arg = c('mashcontrast', 'indep'), ylab='RRMSE')
```
...
colnames(Indep) = c('TPR', 'FPR')
for(t in 1:length(thresh.seq)){
  mashcontrast[t,] = c(CS_S(mashcontrast.model.1, thresh.seq[t], delta)/Tr, N_S(mashcontrast.model.1, thresh.seq[t],delta)/N)
  Indep[t,] = c(CS_S(Indep.model.1, thresh.seq[t], delta)/Tr,  N_S(Indep.model.1, thresh.seq[t],delta)/N)
}
```
