---
title: "John count - individual random effect"
author: "Yuxin Zou"
date: 2018-4-11
output: 
  html_document:
...
```{r model}
design <- model.matrix(~ bact - 1 + extr + rin, data = groups18)
...
* limma
```{r limmaRandomEffect}
corfit = duplicateCorrelation(v, design, block = groups18$ind)
fit = lmFit(v, design, block = groups18$ind, correlation = corfit$consensus.correlation)
```

```{r assign}
beta_class = fit$coefficients[,1:9]
sebeta_class = fit$stdev.unscaled[,1:9] * fit$sigma
t_class = beta_class/sebeta_class
df_class = fit$df.residual
```

```{r save}
saveRDS(list(beta = beta_class, sebeta = sebeta_class, t = t_class, df = df_class), '../output/John18_IndRan_limma.rds')
```
