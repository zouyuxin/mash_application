...
Comparing the estimated V from optim, $M_{\rho}$ and $MV$, $MV$ algorithm computes much faster than $M_{\rho}$, faster than optim. The estimated V from optim and $M_{\rho}$ are very similar. They have smaller error to the true V than $MV$, most of the time. The estimated V from $MV$ has larger log likelihood in the final mash model. In terms of ROC and RRMSE, the estimated V from optim, $M_{\rho}$ and $MV$ performs similarly.
...
2. The result uses algorithm in [estimate cor em](EstimateCorEM3.html). $\rho$ in each iteration is the root of a third degree polynomial.
...
3. The result uses algorithm in [estimate cor em V](EstimateCorMaxMV.html).
