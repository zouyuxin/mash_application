...
If we enlarge the sample sizes, but still $\frac{1}{4}$ rows are missing. There are 16000 samples.

```{r}
set.seed(2018)
data = simple_sims(4000, ncond = 60, err_sd = 0.1)
...
```
```{r, echo=FALSE}
mash.data.missing.1.enlarge = mash_set_data(Bhat=data.large.dev$Bhat, Shat=data.large.dev$Shat, alpha = 1)
U.c = cov_canonical(mash.data.missing.1)

mash.model.missing.1.enlarge = mash(mash.data.missing.1.enlarge, U.c, verbose = FALSE)
barplot(get_estimated_pi(mash.model.missing.1.enlarge), las=2, cex.names = 0.7)
```
