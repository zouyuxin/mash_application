---
title: "John Count 18 Mash - IndRanBayes - EZ"
author: "Yuxin Zou"
date: 2018-4-14
output: 
  html_document:
...
saveRDS(cov_ed(mash.data.ed, U.dd), '../output/JohnCount18_IndRanBayes_mash_EZ_Cov.rds')
```

## Mash model:
```{r, echo=FALSE}
U.ed = readRDS('../output/JohnCount18_IndRanBayes_mash_EZ_Cov.rds')
```

```{r mash, eval=FALSE}
saveRDS(mash(mash.data.diff, c(U.c, U.ed), algorithm.version = 'R'), '../output/JohnCount18_IndRanBayes_mash_model_EZ.rds') 
```

# Result
```{r}
mash.model = readRDS('../output/JohnCount18_IndRanBayes_mash_model_EZ.rds')
```
