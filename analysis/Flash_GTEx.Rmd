...
```{r, eval=FALSE}
flash.loading = flash_set_data(flash_get_l(f_b)[,1:23])
...
Factors = flash_get_f(fl_b)
graphics.off()
par(mar=c(1,1,1,1))
par(mfrow=c(3,2))
# layout(matrix(seq(1,12), 6, 2, byrow = TRUE))
for(i in 1:23){
  barplot(Factors[,i], main=paste0('Factor ',i, ' pve= ', round(flash_get_pve(fl_b)[i],3)), las=2, cex.names = 0.4, col=as.character(gtex.colors), names="")
}
```

There is no latent structure in the flashr loadings matrix.
