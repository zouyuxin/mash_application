---
title: "John Count 18 Mash - IndRan - EE - t"
author: "Yuxin Zou"
date: 2018-4-15
output: 
  html_document:
...
saveRDS(cov_ed(mash.data.ed, U.dd), '../output/JohnCount18_IndRan_mashT_EE_Cov.rds')
```

## Mash model:
```{r, echo=FALSE}
U.ed = readRDS('../output/JohnCount18_IndRan_mashT_EE_Cov.rds')
```

```{r mash, eval=FALSE}
saveRDS(mash(mash.data.diff, c(U.c, U.ed), algorithm.version = 'R'), '../output/JohnCount18_IndRan_mashT_model_EE.rds') 
```

# Result
```{r}
mash.model = readRDS('../output/JohnCount18_IndRan_mashT_model_EE.rds')
```
...
```{r Load 2}
x           <- cov2cor(mash.model$fitted_g$Ulist[["ED_Load_2"]])
x[x > 1]    <- 1
x[x < -1]   <- -1
colnames(x) <- colnames(beta_class)[2:9]
rownames(x) <- colnames(x)
corrplot.mixed(x, tl.pos="d",upper='color',cl.lim=c(0.9,1), upper.col=colorRampPalette(rev(c("#D73027","#FC8D59","#FEE090","#FFFFBF","#E0F3F8","#91BFDB","#4575B4")))(40), tl.cex=1.2)
```
