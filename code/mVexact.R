...
estimate_null_correlation_mle = function(data, Ulist, init, max_iter = 10, tol=1e-4,
  ...
  xUlist = mashr:::expand_cov(m.model$fitted_g$Ulist, m.model$fitted_g$grid, m.model$fitted_g$usepointmass)
  pi_s = get_estimated_pi(m.model, dimension = 'all')
  prior.v <- mashr:::set_prior(length(pi_s), prior)

  # compute loglikelihood
  log_liks = rep(NA,max_iter+1)
  log_liks[1] = mashr:::get_loglik(m.model)
  tracking = list()
...
neglogLik = function(theta, input){
  ...
  lm = mashr:::calc_relative_lik_matrix(data,xUlist)
